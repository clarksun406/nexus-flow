...
#### 2.4.1 复式记账
- 每笔交易产生 `journal_id`，关联借方（DEBIT）和贷方（CREDIT）两条 `ledger_entries`
- 支付确认：贷记商户余额，借记平台收入
- 退款发起：冻结商户可用余额
- 提现请求：冻结商户可用余额

#### 2.4.2 商户余额管理
- **余额表**：`merchant_balances`，按 (merchant_id, token, network) 粒度
- 可用余额（available）+ 冻结余额（frozen）
- 累计入账/出账统计
- 乐观锁并发控制

#### 2.4.3 手续费引擎
- 可配置费率（feeRate）、最低手续费（minFee）
- 支付确认时自动扣费：`入账金额 = 支付金额 × (1 − feeRate)`
- 手续费独立记账条目

#### 2.4.4 提现/结算
- **提现请求表**：`settlement_requests`
- 状态机：`PENDING → PROCESSING → COMPLETED / FAILED`，支持 `CANCELLED`
- 提现时链上转账 + 余额扣减
- 提现失败时余额回滚

#### 2.4.5 事件驱动记账
- `@TransactionalEventListener` 监听现有域事件
- 支付确认 → 自动入账
- 退款状态变更 → 自动冻结/释放
- Fiat Ramp 完成 → 入账/出账
...
| `flow-domain` | 领域核心：聚合根、状态机、值对象、端口（含 settlement 子包） |
| `flow-application` | 用例编排：支付编排、退款、Webhook、结算记账 |
...
- **结算层表**：merchant_balances、ledger_entries、settlement_requests
...
2. **记账结算体系**（R-19）：复式记账、商户余额、手续费引擎、提现结算
3. **真实通道适配**：Coinbase Commerce live验证、BitMart/Binance真实适配器
4. **前端产品化**：按四端架构重建前端工程
5. **生产环境验证**：Docker集成测试、真实链节点、Redis/Kafka
6. **高级功能**：MPC、Gas Abstraction、On/Off Ramp provider集成
...
1. **记账结算体系**：复式记账、商户余额、手续费、提现结算（R-19，全新模块）
2. **商户体系完善**：多租户认证、数据隔离、RBAC（R-16 M4-M7）
3. **产品级前端**：从静态页面迁移到四端React工程（R-17）
4. **生产环境验证**：真实节点、通道、中间件的live验证
5. **高级功能集成**：MPC、Gas Abstraction、On/Off Ramp provider
