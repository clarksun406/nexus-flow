...
#### 2.2.8 结算层功能（Settlement & Accounting）

**FR-SET-001：复式记账**
- **描述**：系统应为每笔余额变动生成借方（DEBIT）和贷方（CREDIT）两条 `ledger_entries`，共享 `journal_id`
- **数据模型**：`ledger_entries` 表
- **记账规则**：

| 场景 | 借方 (DEBIT) | 贷方 (CREDIT) | 余额变动 |
|------|-------------|---------------|----------|
...
**FR-SET-002：商户余额管理**
- **描述**：系统应维护每个商户按 (token, network) 粒度的余额
- **数据模型**：`merchant_balances` 表
  ```sql
...
- **业务规则**：
  - 可用余额不得为负（乐观锁 + 余额校验）
  - 冻结余额 = 提现中金额 + 退款预留金额
  - `total_credited` / `total_debited` 只增不减，用于审计

**FR-SET-003：手续费引擎**
- **描述**：系统应在支付确认时自动扣收手续费
- **配置模型**：
  - `feeRate`：费率（如 0.003 = 0.3%）
  - `minFee`：最低手续费
  - `feeToken`：手续费币种（默认与支付币种相同）
- **业务规则**：
  - 入账金额 = 支付金额 × (1 − feeRate)
  - 手续费独立生成 `FEE` 类型 ledger_entry
  - 费率可按商户配置或全局默认

**FR-SET-004：提现/结算**
- **描述**：商户应能发起提现请求，系统执行链上转账并扣减余额
- **API**：
  - `POST /merchant/settlements`：创建提现请求
  - `GET /merchant/settlements`：提现记录列表
  - `GET /merchant/settlements/{settlementId}`：提现详情
  - `POST /merchant/settlements/{id}/cancel`：取消提现（仅 PENDING 状态）
- **数据模型**：`settlement_requests` 表
  ```sql
  ...
  ```
- **业务规则**：
  - 创建时校验 `available >= amount`
  - 创建即冻结：available -= amount, frozen += amount
  - 链上转账成功 → COMPLETED, frozen -= amount
  - 链上转账失败 → FAILED, frozen -= amount, available += amount
  - PENDING 状态可取消 → CANCELLED, frozen -= amount, available += amount

**FR-SET-005：余额/流水查询 API**
- **描述**：系统应提供余额和记账流水查询接口
- **API**：
  - `GET /merchant/balances`：查询商户全部币种余额
  - `GET /merchant/balances/{token}/{network}`：查询指定币种余额
  - `GET /merchant/ledger-entries`：查询记账流水（分页、按时间/类型/币种筛选）

**FR-SET-006：事件驱动记账**
- **描述**：系统应通过事件监听自动触发记账
- **实现**：`@TransactionalEventListener` 监听现有域事件
- **接入点**：

| 监听事件 | 条件 | 动作 |
|----------|------|------|
| `OrderEvent` | `newStatus == CONFIRMED` | 商户余额入账（available += paidAmount - fee） |
| `PaymentStateChangedEvent` | `newStatus == CONFIRMED` | 自建节点路径：商户余额入账 |
| `OrderEvent` | `newStatus == REFUND_PROCESSING` | 冻结退款金额 |
| `OrderEvent` | `newStatus == REFUNDED` | 释放冻结 |
| `OrderEvent` | `newStatus == REFUND_FAILED` | 释放冻结回可用 |
...
| 方法 | 路径 | 描述 | 认证 |
|------|------|------|------|
| GET | `/merchant/balances` | 查询全部币种余额 | 会话 / API Key |
| GET | `/merchant/balances/{token}/{network}` | 查询指定币种余额 | 会话 / API Key |
| GET | `/merchant/ledger-entries` | 记账流水查询（分页/筛选） | 会话 / API Key |
...
### 6.7 结算请求 SettlementRequest 状态机
...
| 事件类型 | 触发条件 | Payload |
|----------|----------|---------|
| `settlement.balance_credited` | 余额入账 | merchantId, token, network, amount, fee, journalId |
| `settlement.balance_debited` | 余额出账 | merchantId, token, network, amount, journalId |
| `settlement.frozen` | 余额冻结 | merchantId, token, network, amount, reason |
| `settlement.unfrozen` | 余额释放 | merchantId, token, network, amount, reason |
| `settlement.requested` | 提现请求 | settlementId, merchantId, amount, toAddress |
| `settlement.completed` | 提现完成 | settlementId, txHash |
| `settlement.failed` | 提现失败 | settlementId, reason |
| `fiat_ramp.completed` | Fiat Ramp 完成 | rampOrderId, direction, amount, token, network |
...
| R-19 | 记账结算体系 | ⬜ | 复式记账、商户余额、手续费、提现结算全套待实现 |
...
| 风险项 | 当前状态 | 后续动作 |
|--------|----------|----------|
| 记账结算体系 | ⬜ 未开始 | R-19：全套复式记账、余额、手续费、提现 |
...
| Reorg | 区块链重组 |
| Fiat Ramp | 法币-加密货币兑换 |
| 复式记账 | 每笔交易产生借贷两条记录（double-entry bookkeeping） |
| 结算层 | 负责商户余额管理、记账、手续费扣收和提现处理 |
| 可用余额 | 商户可自由使用的余额（available balance） |
| 冻结余额 | 被提现中或退款预留锁定的余额（frozen balance） |
| Journal | 复式记账中关联借贷两条记录的共享 ID |
...
- `nexusflow-settlement-design.md`：结算体系设计
