...
> Current local verification (2026-06-14): `mvn -pl flow-api,flow-cashier -am test` runs 208 passing tests
...
> execution webhooks with dead-letter replay/ignore workflow and opt-in live delivery smoke coverage, Coinbase Commerce REST-capable channel with no-key stub fallback and opt-in live smoke coverage, BitMart/Binance stub beans guarded out of the `prod` profile,
...
| P3 (Testing) | 2 | Unit tests, Integration tests | 🟡 Unit tests (208 passing locally) · 🟡 Integration/live tests present, 14 skipped locally without Docker/live env |
| **Total** | **19** | | |
