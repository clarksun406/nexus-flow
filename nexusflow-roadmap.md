...
| P2-3 | CoinbaseCommerceAdapter | ✅ 支持 Coinbase Commerce REST charge/rate 调用；无 API key 时保留本地 stub fallback，已注册为 Bean |
...
| R-11 | Coinbase Commerce live 验证 | ✅ `CoinbaseCommerceAdapter` 已支持带 `COINBASE_COMMERCE_API_KEY` 的 REST charge 创建和 exchange-rate 查询；无 key 时保留 stub fallback；`CoinbaseCommerceAdapterTest` 使用 mocked transport 覆盖请求头、响应解析、支持币种和 fallback | 配置真实 Coinbase Commerce 凭证执行 live charge/rate smoke，确认 webhook payload/签名语义，并明确退款由外部人工或运营流程处理 |
