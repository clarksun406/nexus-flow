...
| R-13 | Gas abstraction 生产化 | ✅ `GasEstimator` 端口、`StaticGasEstimator` 默认实现、自建节点退款 gas budget 事件字段、`GasBank` 端口和 `GasBankPolicy` 阈值/低 gas 批处理建议已实现；`StaticGasEstimatorTest` / `GasBankPolicyTest` 覆盖 ETH/TRON/BTC 默认估算和 top-up/deferral/batching 决策 | 接入 live gas/fee oracle，实现真实 GasBank 预充值 worker、告警和生产低 gas 批处理调度 |
| R-14 | MPC 钱包集成 | ✅ `MpcSigner` 端口、签名请求/结果 VO、`Wallet.mpcWalletId`、JPA 映射和 V10 Flyway 迁移已实现；钱包恢复/映射测试覆盖该字段 | 实现 Fireblocks/Copper/custom MPC 适配器，将外发交易签名流程切到 MPC，并执行 live signing smoke |
| R-15 | On/Off Ramp 集成 | ✅ `FiatGateway` 端口、quote/order request VO、`FiatRampOrder` 转换跟踪状态机、`FiatRampRepository` 端口、JPA 映射、V11 Flyway 迁移、`FiatRampApplicationService`、`/fiat/ramp` 商户 API 和 `/callback/{gatewayId}/fiat-ramp` HMAC 回调入口已实现；`FiatRampOrderTest` / `JpaFiatRampRepositoryTest` / `FiatRampApplicationServiceTest` / `FiatRampControllerTest` / `CallbackControllerTest` 覆盖 tracking、终态保护、持久化映射、API 编排和 normalized callback | 实现 MoonPay/Ramp/Banxa 真实适配器、KYC/settlement provider-specific payload/signature mapping 和 live settlement smoke |
