...
| P3 | 3 | 3 | 静态页面和 API 聚合已落地；前端自动化/E2E 未单独覆盖 |
...
| 产品级前端 / 控制台体系 | ⬜ 当前为静态 HTML 页面和本地配置，缺登录、会话、权限、前端构建、状态管理和 E2E；分端规划见 `nexusflow-frontend-design.md` |
| 商户体系 / 多租户认证 | ⬜ 当前只有请求中的 `merchantId` 和全局 `X-API-Key`；缺商户账号、商户级 API key、权限和数据隔离；设计见 `nexusflow-merchant-design.md` |
| RBAC 权限服务 | 🟡 `flow-permission` 已纳入根 Maven，服务端 service token 鉴权和 scope 唯一约束已补；仍缺身份来源、商户映射和业务接口接入 |
...
| R-16 | 商户体系 / 商户级认证 | 🟡 M1-M3 已落地：`merchant_profiles` / `merchant_credentials` / `merchant_webhook_configs` 表和 V12 Flyway 迁移；`MerchantApiKey` / `MerchantCredentialRepository` 端口和 JPA 适配器；`ApiKeyHasher` (SHA-256) 和 `ApiKeyAuthFilter` 改造为商户 key 解析，设置 `merchantId` / `merchantCode` / `authSource` request attributes；`MerchantRequestGuard.requireMatchingMerchant()` 校验 POST body 和 GET response 的 `merchantId`；`MerchantRequestGuard.requireGlobalAccess()` 和 `ApiKeyAuthFilter` 内联检查限制商户 key 访问 `/ops/*` 和 `/crypto/*`；`OrderResponse` 新增 `merchantId` 字段支持 GET 所有权校验；`GlobalExceptionHandler` 对 UNAUTHORIZED 返回 401。全局 `nexusflow.api.key` 保留为 dev/test fallback。测试覆盖：`ApiKeyAuthFilterTest` / `ApiKeyHasherTest` / `MerchantRequestGuardTest` / `PayControllerTest` / `FiatRampControllerTest` / `JpaMerchantCredentialRepositoryTest` | 继续 M4 (webhook 持久化)、M5 (Merchant Portal 会话)、M6 (Ops/Admin + RBAC)、M7 (生产禁用全局 key)；补商户管理 API、key 轮换/禁用、审计、商户数据隔离查询 |
| R-17 | 产品级前端 / 控制台体系 | ⬜ 当前 `flow-cashier` 和 `frontend` 主要是静态 HTML/CSS/JS 页面：`checkout.html`、`merchant.html`、`ops.html` 与早期页面 Demo。商户和运营配置依赖浏览器 `localStorage`，没有登录/会话、角色权限、商户资料、API key 管理、Webhook 配置持久化、统一导航、前端路由、构建产物、错误边界、自动化 E2E 或真实用户工作流闭环。已新增 `nexusflow-frontend-design.md`，明确正式前端应拆为 Checkout、Merchant Portal、Ops Console、Admin Console 四个端 | 按 `nexusflow-frontend-design.md` 推进：建立 `frontend` workspace、共享 UI/API client/auth 包；Checkout 增加 checkout token、品牌配置和异常状态 E2E；Merchant Portal 接入 R-16 商户体系，管理订单、退款、API key、Webhook、团队和报表；Ops Console 接入内部登录/RBAC，覆盖通道、orphan、dead letter、对账和风险处置；Admin Console 管理商户生命周期、凭证、RBAC、provider 配置和审计；补齐构建、部署、环境配置、可观测性、Playwright/Cypress E2E 和跨浏览器回归 |
| R-18 | RBAC 权限服务接入 | 🟡 `flow-permission/` 已纳入根 Maven，包含 `flow-permission-server` / `flow-permission-client`；server 有 `permissions`、`roles`、`role_permissions`、`user_roles` 表和角色/权限 CRUD，client 有 `@CheckPermission` AOP、HTTP client 和 Caffeine cache；已统一 Maven 父 POM/Spring Boot 版本，补 `Authorization: Bearer` service token 过滤器，修正 `user_roles.scope_id` nullable unique 为 partial unique indexes，并新增 service token filter 单测。client 仍依赖 request attribute 中已有 `userId`/`merchantId`，但主系统 API key filter 尚不会设置；业务接口尚未接入 `@CheckPermission` | 先完成 R-16 商户身份体系和 `nexusflow-merchant-design.md` 中的认证上下文，明确 `userId`、`merchantId`、商户 UUID 与外部 merchant code 映射；主系统认证成功后写入 request attributes，再按需引入 permission client 和 `@CheckPermission`；补充 permission server/client 更完整的 controller/service 测试、降级策略和缓存失效机制 |
