...
| # | 任务 | 状态 |
|---|------|------|
| P1R-1 | BinancePayAdapter — stub 实现，注册为 Bean | ✅ |
...
> 仍未做：执行层 `CryptoPayment.callbackUrl` 实际投递、TronAdapter 真实扫块（`scanNewBlocks`）、`createPayment` 全量幂等、链上已存在交易但系统无支付记录时的 catch-up 补偿、真实 ETH/BTC/TRON 节点端到端验证。Testcontainers 集成测试已补充，但无 Docker 环境时会自动跳过。地址池补充已实现，但生产环境必须配置 `ADDRESS_POOL_SEED_MNEMONIC` 才会自动补池。
...
- **API 认证**：新增 `ApiKeyAuthFilter`（`X-API-Key` 头），保护 `/pay/**`、`/crypto/**`、`/refund/**` 端点。`/callback/**` 使用 HMAC、`/cashier/**` 面向终端用户、`/actuator/**` 面向运维，各自跳过。
- **回调签名验证**：新增 `CallbackHmacFilter` + `HmacSignatureVerifier`，验证 `/callback/**` 端点的 `X-Signature` 头（HMAC-SHA256），每个通道独立密钥。
- **加密密钥检查**：`AesGcmConfig` 在 `nexusflow.encryption.key` 未配置时默认拒绝启动，需显式设置 `nexusflow.encryption.allow-generated-key=true` 才允许生成临时密钥（仅限开发环境）。注意：2026-06-14 审查发现 `application.yml` 当前通过 `${ALLOW_GENERATED_KEY:true}` 把配置默认值放宽为 `true`，生产前需改为默认 `false` 或由生产环境显式覆盖。
...
- **@Positive 校验修复**：`CreateOrderRequest.amountFiat` 和 `RefundRequestDto.refundAmountFiat` 使用 `@Positive` 标注在 `String` 类型上无效。改为 `@Pattern(regexp = "^[0-9]+(\\.[0-9]+)?$")` 正则校验。
...
- **BinancePayAdapter**：新增 `BinancePayAdapter` stub 实现（`flow-infra/.../adapter/binance/`），支持 USDT(TRC20/ERC20/BEP20) + BTC，注册为 Spring Bean。与 `BitMartAdapter` 并列，由 `DefaultChannelRouter` 自动选择。
...
- **集成测试（Testcontainers）**：新增 `NexusFlowApplicationIT`（`flow-api`），`@SpringBootTest` + `@Testcontainers(disabledWithoutDocker=true)` + PostgreSQL 容器。验证 Spring 上下文加载和 `OrderRepository` JPA 读写。无 Docker 环境时自动 skip。
- **前端页面**：从 `E:\gitee\channelpay-pages` 复制到 `frontend/` 目录和 `flow-cashier/src/main/resources/static/`，包含 7 个收银台页面和 1 个交互式演示。
...
- **测试**：新增 `JpaPaymentRepositoryTest`、`JpaWalletRepositoryTest`，并扩展 `NexusFlowApplicationIT` 覆盖执行层支付和钱包 JPA round trip。`mvn test` 通过；本机无 Docker，4 个 Testcontainers 用例自动跳过。
...
- **测试验证**：新增 `BlockchainCircuitBreakerTest`、`EthereumAdapterTest`、`BitcoinAdapterTest`、`BlockchainScannerTest`、`JpaAddressPoolRepositoryTest`、`JpaMnemonicStoreTest`；扩展 `KeyGeneratorTest` 和支付服务测试。2026-06-14 本机 `mvn test` 通过，134 个测试通过；4 个 Testcontainers 用例因本机 Docker 不可用自动跳过。

---

## 十三、生产前剩余风险 / 未验证项（2026-06-14）

P1 代码项已基本落地，但以下事项仍不应按生产闭环处理：

| # | 项目 | 当前状态 | 后续动作 |
|---|------|----------|----------|
| R-1 | TRON 自动扫块 | `TronAdapter.scanNewBlocks()` 仍是显式 stub，仅 `getCurrentBlockHeight` / `getConfirmations` / `isHealthy` 有实现 | 接入 TronGrid event-info 或全节点日志解析，覆盖 USDT_TRC20 入账扫描 |
| R-2 | Docker 集成测试实跑 | `NexusFlowApplicationIT` 已存在，但本机无 Docker 时 4 个用例自动 skip | 在 CI 或本地 Docker 环境跑通 PostgreSQL/Testcontainers/Flyway/Spring 上下文 |
| R-3 | 加密密钥生产默认值 | `AesGcmConfig` 代码默认拒绝临时密钥，但 `application.yml` 当前 `${ALLOW_GENERATED_KEY:true}` 会默认放行 | 将默认值改为 `false`，生产强制配置 `ENCRYPTION_KEY` |
| R-4 | 地址池并发分配 | `findFirstAvailableByChain` 未使用数据库行锁或 `SKIP LOCKED`，高并发下可能抢同一地址 | 增加悲观锁 / `FOR UPDATE SKIP LOCKED` 或乐观锁冲突自动重试 |
| R-5 | 执行层回调投递 | `CryptoPayment.callbackUrl` 已持久化，但执行层确认后的商户回调投递未闭环 | 复用 Webhook 投递能力或新增执行层 callback job |
| R-6 | createPayment 全量幂等 | 当前按 `orderId` 做存在性检查，未形成完整请求幂等语义 | 明确 idempotency key / 参数一致性校验 / 重放响应策略 |
| R-7 | Missing-event catch-up | 扫链发现链上交易但系统无支付记录时仅忽略，未做补偿建单或告警 | 设计 orphan transaction 记录、告警和人工/自动认领流程 |
| R-8 | 真实节点端到端验证 | ETH/BTC 适配器有 mocked transport 单测，TRON confirmations 有解析单测；未连真实节点验证 | 用真实或本地节点环境验证 ETH/BTC/TRON 扫描、确认数、reorg 和异常恢复 |
