...
| R-15 | On/Off Ramp 集成 | ✅ `FiatGateway` 端口、quote/order request VO、`FiatRampOrder` 转换跟踪状态机、`FiatRampRepository` 端口、JPA 映射和 V11 Flyway 迁移已实现；`FiatRampOrderTest` / `JpaFiatRampRepositoryTest` 覆盖 fiat→crypto/crypto→fiat settlement reference tracking、终态保护和持久化映射 | 实现 MoonPay/Ramp/Banxa 真实适配器、KYC/settlement webhook、API 编排和 live settlement smoke |
