...
本 Roadmap 区分四种完成度，避免把“代码已落地”误读成“生产闭环已完成”：

| 标记 | 含义 |
|------|------|
| ✅ 代码已落地 | 领域模型、接口、应用服务、持久化、配置或离线测试已经实现 |
| 🧪 已离线验证 | 单元测试、Mock transport、MockMvc 或本地模块测试已覆盖 |
| 🟡 待 live 验证 | 需要 Docker、真实节点、真实 broker、真实 provider 凭证或生产拓扑验证 |
| ⬜ 生产待补 | 仍需真实供应商适配器、签名/广播/结算链路、生产调度、告警或运营策略 |

因此下文 P0-P3 的进度表示**代码范围完成度**；生产 readiness 以第十三节“生产前剩余风险 / 未验证项”为准。
...
│
├── frontend-checkout/                   # 买家收银台静态资源模块，打包 Vue checkout 产物
├── frontend-merchant/                   # 商户门户静态资源模块，打包 Vue merchant 产物
├── frontend-ops/                        # 运营控制台静态资源模块，打包 Vue ops 产物
├── frontend-admin/                      # 平台管理端静态资源模块，打包 Vue admin 产物
└── frontend/                            # Vue 3 + Vite + TypeScript workspace
    ├── apps/
    │   ├── checkout/
    │   ├── merchant/
    │   ├── ops/
...
| P0-9 | frontend-checkout 收银台前端模块（checkout.html / Vue checkout app） | ✅ |
| P0-10 | 编译验证（mvn install -DskipTests） | ✅ |

### ═══════════════════════════════════════════
### 🔴 P0-S — 安全与正确性修复 ✅ 代码已落地 (6/6)
### ═══════════════════════════════════════════
...
### ═══════════════════════════════════════════
### 🔵 P3 — 前端 ✅ Vue 工程化与分端入口已落地
### ═══════════════════════════════════════════

| # | 任务 | 状态 |
|---|------|------|
| P3-1 | 收银台（Cashier UI）— 买家支付页，二维码/地址展示，状态轮询 | ✅ `frontend/apps/checkout` Vue app 已落地，覆盖订单加载、地址生成、二维码/复制、倒计时和状态轮询；旧 `checkout.html` 仅作 legacy fallback |
...
| 阶段 | 任务数 | 代码已落地 | 状态口径 |
|------|--------|------------|----------|
| P0 | 10 | 10 | 编排核心代码/迁移/API/静态收银台已落地 |
| P0-S | 6 | 6 | 安全与正确性修复已落地 |
| P1 | 10 | 10 | 生产加固代码和单元测试已落地 |
| P1-F | 6 | 6 | 缺陷修复和测试补全已落地 |
...
| ETH / BTC / TRON 链上适配 | 🟡 离线测试覆盖，真实节点和长窗口扫描待验证 |
| BitMart / Binance 通道 | ⬜ 当前仍为非 prod stub，真实 REST/签名/回调/退款未实现 |
| Coinbase Commerce | 🟡 REST charge/rate 已实现，live charge/webhook/退款运营流程待验证 |
| 自建节点退款 | 🟡 事件和 gas budget 已发出，链上签名/广播由外部 worker 或真实节点环境完成 |
| Kafka / Redis | 🟡 代码和 opt-in smoke 存在，broker/Redis 生产拓扑待验证 |
| MPC | ⬜ 端口和 generic HTTP signer 已实现，真实 provider 和交易签名路径未闭环 |
| Gas abstraction | ⬜ 静态估算和策略已实现，live fee oracle、真实 GasBank、调度和告警未闭环 |
| On/Off Ramp | ⬜ 通用 HTTP gateway 和 tracking 已实现，MoonPay/Ramp/Banxa 官方语义和 live settlement 未闭环 |
...
- **前端页面**：早期页面已从 `E:\gitee\channelpay-pages` 引入；当前静态资产已拆分到 `frontend-checkout` / `frontend-merchant` / `frontend-ops` / `frontend-admin`，原始 Demo 已迁入 `frontend/legacy-static/`；同时已建立 `frontend/` Vue workspace 作为后续正式前端源码。
...
| R-16 | 商户体系 / 商户级认证 | 🟡 M1-M3 已落地（同前）；M5 基础已推进：`merchant_users` / `merchant_user_memberships` 表和 V13 Flyway 迁移已创建；`MerchantUser` / `MerchantUserMembership` 领域实体和 JPA 仓储已实现；`AuthController` / `AuthService` / Spring Session 迁移已接入 `/auth/login`、`/auth/logout`、`/auth/me`、`/auth/active-merchant`；`ApiKeyAuthFilter` 支持商户 key userId 派生和 session 回退；`MerchantUserProvisioningService` 启动时自动同步商户和 ops 用户角色到 `flow-permission`；新增用户映射、角色同步和认证相关测试 | 继续补 API key scopes、key 轮换/禁用/审计、商户管理 API、商户数据隔离查询、生产禁用全局 key，以及真实后端 E2E |
| R-17 | 产品级前端 / 控制台体系 | 🟡 FR-0~FR-4 已推进（截至 2026-07-11）：`flow-cashier` 已拆为 `frontend-checkout`（买家收银台）、`frontend-merchant`（商户门户）、`frontend-ops`（运营控制台）、`frontend-admin`（平台管理端）4 个独立 Maven 模块；`frontend/` 下已建立 Vue 3 + Vite + TypeScript workspace、共享 API/UI 包和 4 个 app；Checkout 覆盖订单加载、地址生成、QR、复制、倒计时和状态轮询；Merchant Portal 覆盖登录、商户切换、创建订单、订单查询、退款提交和最近订单缓存；Admin Console 覆盖登录、角色管理、权限分配、用户角色查询、授权和撤销；Ops Console 覆盖登录、通道/订单/对账 dashboard、orphan resolve/compensate/ignore、webhook dead letter replay/ignore；`npm run verify:e2e` 已覆盖 typecheck/build/prune/smoke/Playwright Chromium mock E2E；GitHub Actions 已在 Maven 构建前执行前端验证并上传 `dist-app` artifact；`frontend-*` Maven 模块已把对应 Vite `dist-app` 接入 `static/app/` 并通过打包验证。**未闭环：** 真实后端联调 E2E、正式部署策略、灰度/回滚、前端观测，以及 API key/Webhook/商户生命周期等产品页 | 后续补真实 `flow-api` 联调 E2E、部署链路和观测 |
| R-18 | RBAC 权限服务接入 | ✅ `flow-permission-client` 已添加为 `flow-api` 依赖；`ApiKeyAuthFilter` 设置 `userId` request attribute（商户 key 用 `UserIdMapper.toUuid()` 派生确定性 UUID，全局 key 用固定 ops UUID `00000000-0000-0000-0000-000000000001`）；`@CheckPermission` 已标注所有 controller 方法（商户端点用 MERCHANT scope，运营端点用 SYSTEM scope）；`PermissionClient` 新增 `getRoleByCode()` / `grantRole()` / `hasRoles()` 方法；`MerchantUserProvisioningService` 启动时自动同步角色（idempotent，permission server 不可用时 graceful degradation）；`GlobalExceptionHandler` 新增 403 handler；`application.yml` 新增 `permission.*` 配置段（默认 `enabled=false`）；flow-permission-server 新增 V5 migration seed ops user；新增 `UserIdMapperTest` / `MerchantUserProvisioningServiceTest` / `ApiKeyAuthFilterTest` userId 测试。**2026-08-29 修复**：完整上下文首次真实启动时暴露并修复 3 个装配 bug——`NexusFlowApplication` 补 `@EnableJpaRepositories`/`@EntityScan`（infra 包的实体/仓储从未被扫描，应用无法完整启动）；`PermissionClientAutoConfiguration` 移除 `@ConditionalOnProperty`（`enabled=false` 默认值下 PermissionClient bean 缺失导致启动失败，改为常驻 + 自我降级）；`CheckPermissionAspect` 请求属性名对齐 `nexusflow.userId`/`nexusflow.merchantId`（原读 `userId`/`merchantId` 导致所有受保护端点恒 403） | 补充 permission server/client 更完整的 controller/service 测试、fail-closed 策略、缓存失效机制、merchantId UUID 收敛和真实联调覆盖 |
| R-19 | 零依赖本地启动 / 双数据库 DDL | ✅ 新增 `h2` profile（`application-h2.yml`）：内嵌 H2（PostgreSQL 兼容模式）上一键本地启动，无需 PostgreSQL/Redis/Kafka；Flyway 同一套 `db/migration` 脚本在双库执行——V9/V11/V13 改为双库可移植语法（对 PG 语义等价，存量 PG 库需 `flyway repair` 或重建）；`H2MigrationCompatibilityTest` 守护迁移脚本双库兼容性；`LocalSeedDataRunner` 幂等种子演示数据（商户 `DEMO-001`、API key `nexusflow-local-api-key`、门户用户 `demo@nexusflow.local`/`demo1234`）；`LocalProfileH2BootTest` 全上下文 H2 启动 + 认证链路验证；H2 console `/h2-console`；顺带修复 `JpaMerchantCredentialRepository` 多实体元组强转 bug（改为单实体查询 + profile 二次查找）。**注意**：H2 仅限本地开发；地址池 `FOR UPDATE SKIP LOCKED` 等 PG 专有优化未在 H2 验证，生产仍以 PostgreSQL 为准 | 新迁移需保持双库可移植语法（守护测试会拦截）；后续可评估 Testcontainers 用 H2 替代/补充 PG 集成测试 |

R-17 补充（2026-07-11）：GitHub Actions 已在 Maven `clean install` 前执行 `frontend/` 下的 `npm ci` + `npm run verify:e2e`，并上传 `frontend/apps/*/dist-app` artifact；`frontend-*` Maven 模块通过 `dist-app -> static/app` 打包 Vue 产物；Playwright Chromium E2E 已覆盖 Checkout/Merchant/Ops/Admin 四端构建产物能通过 HTTP 启动，并通过 mock API 跑通 checkout 生成地址、merchant 登录建单、ops orphan resolve、admin 创建角色并分配权限。剩余缺口收敛为真实后端联调 E2E、正式部署策略、灰度/回滚和前端观测。

---

## 十四、RBAC 权限接入记录（2026-07-09）

将 `flow-permission` 权限模块接入 `flow-api`，使 `@CheckPermission` 在全部 controller 上生效：

- **依赖接入**：`flow-api/pom.xml` 新增 `flow-permission-client` 依赖；`application.yml` 新增 `permission.*` 配置段（`server-url`、`service-token`、`enabled`，默认 `false`）。
- **userId 注入**：`ApiKeyAuthFilter` 在认证成功后设置 `nexusflow.userId` request attribute。商户 API key 使用 `UserIdMapper.toUuid(merchantId)` 派生确定性 UUID v5；全局 API key 使用固定 ops UUID `00000000-0000-0000-0000-000000000001`。`MerchantAuthContext` 新增 `USER_ID_ATTRIBUTE` 和 `userId()` 方法。
- **@CheckPermission 标注**：全部 8 个 controller、18 个方法已标注。商户端点（Pay/Refund/FiatRamp）使用默认 `scopeType="MERCHANT"`；运营/执行端点（Ops/Orphan/WebhookDL/Payment）使用 `scopeType="SYSTEM"`。
- **角色自动同步**：`MerchantUserProvisioningService` 监听 `ApplicationReadyEvent`，启动时为 ops 用户（SYSTEM scope）和每个商户（MERCHANT scope）同步角色到 `flow-permission` 的 `user_roles` 表。幂等执行，permission server 不可用时 graceful degradation。
- **PermissionClient 扩展**：新增 `getRoleByCode()`（查角色 ID）、`grantRole()`（授权，幂等）、`hasRoles()`（检查是否已有角色）、`isEnabled()` 方法。
- **403 处理**：`GlobalExceptionHandler` 新增 `CheckPermissionAspect.PermissionDeniedException` → HTTP 403 + `ErrorCode.FORBIDDEN` (NF-0006)。
- **数据库**：flow-api 新增 V13 迁移（`merchant_users` + `merchant_user_memberships` 表）；flow-permission-server 新增 V5 迁移（seed ops 用户 OPS_ADMIN 角色）。
- **领域层**：新增 `MerchantUser`、`MerchantUserStatus`、`MerchantUserMembership`、`MerchantUserRepository`、`MerchantUserMembershipRepository`、`MerchantProfileRepository`（domain 端口）；新增对应 JPA 实体和仓储适配器。
- **测试**：新增 `UserIdMapperTest`(4)、`MerchantUserProvisioningServiceTest`(6)；`ApiKeyAuthFilterTest` 新增 2 个 userId 属性测试。

> **未闭环项：** API key scopes 细粒度控制、key 轮换/禁用/审计、商户数据隔离查询、merchantId 统一 UUID 格式、PermissionClient fail-closed 策略、缓存失效机制、真实后端联调 E2E。
