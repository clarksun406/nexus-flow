...
| R-13 | Gas abstraction 生产化 | ✅ `GasEstimator` 端口、`StaticGasEstimator` 默认实现和自建节点退款 gas budget 事件字段已实现；`StaticGasEstimatorTest` 覆盖 ETH/TRON/BTC 默认估算 | 接入 live gas/fee oracle，实现 GasBank 预充值/告警和低 gas 批处理策略 |
