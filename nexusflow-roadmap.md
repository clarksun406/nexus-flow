...
| R-5 | 执行层回调投递 | ✅ `PaymentApplicationService` 状态变更后调用 `WebhookService.notifyCryptoPayment()`；`@EnableAsync` 启用异步投递；失败/SSRF 拦截会写入 `webhook_dead_letters`（V8 Flyway + JPA store），复用 retry/HMAC/SSRF 防护 | 后续可在运营端增加 dead-letter 重放/关闭工作流 |
