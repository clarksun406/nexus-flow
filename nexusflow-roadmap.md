...
| RBAC 权限服务 | ⬜ `nexus-flow-permission` 已有独立原型，但未接入主工程、缺服务端鉴权、身份来源、商户映射和测试 |
...
| R-18 | RBAC 权限服务接入 | 🟡 新增 `nexus-flow-permission/` 独立 Maven 项目，包含 server/client 两个子模块；server 有 `permissions`、`roles`、`role_permissions`、`user_roles` 表和角色/权限 CRUD，client 有 `@CheckPermission` AOP、HTTP client 和 Caffeine cache。当前未加入根 `pom.xml`，Spring Boot 版本与主项目不一致；server 未验证 `Authorization: Bearer` service token；client 依赖 request attribute 中已有 `userId`/`merchantId`，但主系统 API key filter 尚不会设置；`user_roles.scope_id` 使用 nullable 唯一约束存在重复全局 scope 风险；缺单元/集成测试 | 先完成 R-16 商户身份体系和 `nexusflow-merchant-design.md` 中的认证上下文，明确 `userId`、`merchantId`、商户 UUID 与外部 merchant code 映射；统一 Maven 父 POM 和 Spring Boot 版本；为 permission server 增加 service-token 鉴权和管理端权限保护；修正 `user_roles` 唯一约束为 partial unique indexes；主系统认证成功后写入 request attributes，再按需引入 permission client 和 `@CheckPermission`；补充 server/client 测试、降级策略和缓存失效机制 |
