...
> 仍未做：TronAdapter 真实扫块（`scanNewBlocks`）、真实 ETH/BTC/TRON 节点端到端验证，以及 orphan transaction 的告警/自动补偿流程。Testcontainers 集成测试已补充，但无 Docker 环境时会自动跳过。地址池补充已实现，但生产环境必须配置 `ADDRESS_POOL_SEED_MNEMONIC` 才会自动补池。
...
| R-4 | 地址池并发分配 | ✅ `findFirstAvailableByChain` 使用 PostgreSQL `FOR UPDATE SKIP LOCKED` 锁定可用地址 | 后续在真实 PostgreSQL 下做并发压力测试 |
| R-5 | 执行层回调投递 | ✅ `PaymentApplicationService` 状态变更后调用 `WebhookService.notifyCryptoPayment()`，复用现有 retry/HMAC/SSRF 防护 | 后续可补 dead-letter / 异步投递 |
| R-6 | createPayment 全量幂等 | ✅ `Idempotency-Key`/`X-Idempotency-Key` + 请求 hash + `idempotency_keys` 响应缓存/重放已实现 | 后续可补 HTTP E2E 测试 |
| R-7 | Missing-event catch-up | 🟡 扫链发现自有地址但无 PENDING payment 时会持久化 `orphan_transactions` 记录并去重；`/crypto/orphan-transactions` 支持查询、resolve、ignore | 继续补告警和自动补偿建单流程 |
