...
| R-16 | 商户体系 / 商户级认证 | ⬜ 当前商户维度只体现在请求/订单里的 `merchantId`，认证是单个全局 `X-API-Key`；`merchant.html` 的 API key/回调地址配置仅保存在浏览器本地。尚无商户表、商户级 API key、key hash/轮换/禁用、`merchantId` 与 key 绑定校验、商户 webhook 配置持久化、权限模型或多租户数据隔离。已新增 `nexusflow-merchant-design.md`，明确商户主数据、商户用户、商户级 API key、Webhook 配置、审计、数据隔离和迁移阶段 | 按 `nexusflow-merchant-design.md` 推进：新增 `MerchantProfile` / `MerchantCredential` / `MerchantWebhookConfig` 模型与迁移；实现商户级 API key hash 存储、认证解析和 request attributes；校验请求 `merchantId` 与认证商户绑定；持久化商户 webhook 配置；补充商户数据隔离、key 轮换/禁用、审计和测试；全局 `nexusflow.api.key` 仅保留为 dev/test fallback 或迁移期选项 |
