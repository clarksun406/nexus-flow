...
| R-15 | On/Off Ramp 集成 | ✅ `FiatGateway` 端口、quote/order request VO、`FiatRampOrder` 转换跟踪状态机和 `FiatRampRepository` 端口已实现；`FiatRampOrderTest` 覆盖 fiat→crypto/crypto→fiat settlement reference tracking 和终态保护 | 实现 MoonPay/Ramp/Banxa 真实适配器、KYC/settlement webhook、JPA 映射/API 编排和 live settlement smoke |
