...
> 仍未做：TronAdapter 真实扫块（`scanNewBlocks`）、真实 ETH/BTC/TRON 节点端到端验证。Testcontainers 集成测试已补充，但无 Docker 环境时会自动跳过。地址池补充已实现，但生产环境必须配置 `ADDRESS_POOL_SEED_MNEMONIC` 才会自动补池。Orphan transaction 告警与补偿建单已补齐，自动补偿需显式启用 `ORPHAN_AUTO_COMPENSATION_ENABLED=true`。
