...
> 仍未做：TronAdapter 真实扫块（`scanNewBlocks`）、真实 ETH/BTC/TRON 节点端到端验证，以及 orphan transaction 的告警/人工或自动认领流程。Testcontainers 集成测试已补充，但无 Docker 环境时会自动跳过。地址池补充已实现，但生产环境必须配置 `ADDRESS_POOL_SEED_MNEMONIC` 才会自动补池。
...
| R-7 | Missing-event catch-up | 🟡 扫链发现自有地址但无 PENDING payment 时会持久化 `orphan_transactions` 记录并去重 | 继续补告警、人工/自动认领和补偿建单流程 |
