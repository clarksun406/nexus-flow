...
| P1R-6 | 集成测试（Testcontainers/Docker）— `NexusFlowApplicationIT`（PostgreSQL 容器 + Spring Boot 上下文），无 Docker 时自动 skip | ✅ |
...
- **加密密钥检查**：`AesGcmConfig` 在 `nexusflow.encryption.key` 未配置时默认拒绝启动，需显式设置 `nexusflow.encryption.allow-generated-key=true` 才允许生成临时密钥（仅限开发环境）。
